...
const Search = ({loading, setLoading, setResults, setHasSearched}) => {
  ...
  return (
    <div className='flex flex-row justify-center items-center border-2 border-blue-200 rounded-md p-2 text-blue-900'>
        ...
        <input id='search' value={term} onChange={handleSearchInputChanges}
          className='py-1 px-2 mr-2 w-2/3 text-blue-900 font-bold outline-none border-blue-900' />
        </label>
        <button type='submit' onClick={(evt)=> term && !loading && callSearchFunction(evt)}
        className='bg-blue-800 text-white font-bold py-1 px-2 w-1/3 rounded-md focus:ring'>Search</button>
    </div>
  )
}
